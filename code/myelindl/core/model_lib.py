...
MODEL_TYPE_IMAGE_CLASSIFICATION = 'img'
MODEL_TYPE_OBJECT_DETECTION = 'obj'
MODEL_TYPE_ADVANCE = 'adv'
...
def get_model(network_dir, file_name="network.py", params=None):
    if not network_dir:
        raise ValueError('network_dir not defined')
    model_file = os.path.join(network_dir, file_name)
    if not os.path.isfile(model_file):
        raise ValueError('Could not find %s in %s for model file' % (model_file, network_dir))
    ...
    return UserModel(params=params)


class AdvModel(object):
  def __init__(self, job_dir, params=None):
    try:
      model_file = os.path.join(job_dir, 'network.py')
      m = imp.load_source('AdvModel', model_file)
      self.model_type = MODEL_TYPE_ADVANCE
      self.train_input_fn = m.train_input_fn
      self.eval_input_fn = m.eval_input_fn
      self.network_fn = m.network_fn
      self.fetches_fn = m.fetches_fn
      self.loss_fn = m.loss_fn
      self.params = params
      ...
      tf.logging.fatal('{}, {}'.format(e.message, traceback.print_exc()))
      exit(-1)
  ...
  def build_network(self, inputs, phase_train=True, nclass=1001):
    ...
    ## network_fn(inputs, labels, training=True)
    logits = self.network_fn(features, phase_train, self.data_format, self.data_type)
    if self.data_type == tf.float16:
      logits = tf.cast(logits, tf.float32)
    return BuildNetworkResult(
        logits=logits, extra_info=None)
  ...
  def accuracy_function(self, inputs, logits):
    _, labels = inputs
    ## fetches_fn(logits, labels)
    fetches = self.fetches_fn(logits, labels)
    return fetches
  ...
  def get_input_shapes(self, subset):
    del subset  # fake data
    return [[self.batch_size, 8, 8, 1],
            [self.batch_size]]
...
class NNModel(Model):
  """Base model configuration for NN benchmarks."""
  ...
  def __init__(self,
    ...
    super(NNModel, self).__init__(
      model, model_type, batch_size, learning_rate, fp16_loss_scale, params=params)
  ...
  def accuracy_function(self, inputs, logits):
    ...
    return {'top_1_accuracy': top_1_op, 'top_5_accuracy': top_5_op,
            constants.UNREDUCED_ACCURACY_OP_PREFIX+'softmax': softmax}

  def postprocess(self, results):
    """Postprocess results returned from model in Python."""
    if 'softmax' in results:
        self.softmax.extend(results['softmax'].tolist())
    return results
